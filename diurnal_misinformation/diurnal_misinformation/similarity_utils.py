...
from .fourier_utils import get_increased_activity, similarity_measures
...
def similarity_routine(activity, ratio, increased_activity=None, index_slice=pd.IndexSlice[:], compare_to=None):
    curve_stats = activity.apply(find_curve_features, axis=1)
    curve_stats['onset increased activity'] = get_increased_activity(activity) if increased_activity is None else increased_activity
    if compare_to is None:
    ...
    return SignalDistances(
        ...
        distance_ratio_by_activity_curve_features = mean_distance_for_shifts(ratio.loc[index_slice], curve_stats.loc[index_slice], similarity_measures, compare_to=compare_to),
        ...
        g_ratio_by_activity_curve_features = plot_daily_activities(ratio, shift_by=curve_stats.loc[index_slice], title="Ratios of potentially disinformative content aligned by activity curve features")
    )
